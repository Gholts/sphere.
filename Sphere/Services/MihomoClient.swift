...
struct MihomoClient: ProxyBackendClient {
    ...
    func reloadConfig() async throws {
        ...
        try await requestNoBody(path: "/configs", query: [URLQueryItem(name: "force", value: "true")], method: "PUT", body: body)
    }
    ...
    func refreshProxyProvider(_ name: String) async throws {
        try await requestNoBody(path: "/providers/proxies/\(escaped(name))", method: "PUT")
    }
    ...
    func refreshRuleProvider(_ name: String) async throws {
        try await requestNoBody(path: "/providers/rules/\(escaped(name))", method: "PUT")
    }
    ...
    func upgradeCore(channel: CoreUpdateChannel) async throws {
        try await requestNoBody(path: "/upgrade", query: [URLQueryItem(name: "channel", value: channel.rawValue)], method: "POST")
    }
    ...
    private func webSocketRequest(path: String, query: [URLQueryItem]) throws -> URLRequest {
        var request = try URLRequestFactory.request(profile: profile, path: path, query: query, timeoutInterval: requestTimeout)
