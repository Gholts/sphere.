...
enum PreviewFixtures {
    static func app() -> AppModel {
        ...
        app.proxyProviders = [
            ProxyProvider(name: "main", vehicleType: "HTTP", expireAt: Date().addingTimeInterval(86400 * 30), usedBytes: 10 * 1024 * 1024 * 1024, totalBytes: 100 * 1024 * 1024 * 1024)
        ]
        app.rules = [
            RuleItem(type: "DOMAIN-SUFFIX", payload: "example.com", proxy: "GLOBAL")
        ]
        app.connections = ConnectionsSnapshot(uploadTotal: 2048, downloadTotal: 8192, connections: [
            ConnectionInfo(id: "1", metadata: ConnectionMetadata(network: "tcp", type: "HTTP", sourceIP: "192.168.1.20", destinationIP: "93.184.216.34", host: "example.com", process: nil), upload: 2048, download: 8192, start: nil, chains: ["GLOBAL", "Japan"], rule: "DOMAIN-SUFFIX", rulePayload: "example.com")
        ])
        return app
    }
}
