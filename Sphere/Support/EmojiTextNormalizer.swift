...
enum EmojiTextNormalizer {
    ...
    private static func parseUnicodeEscape(in value: String, at index: String.Index) -> ParsedText? {
        ...
        case "u":
            ...
            guard let first = parseHexValue(in: value, afterPrefixAt: index, prefixLength: 2, hexLength: 4) else {
                return nil
            }
            ...
            guard !isSurrogate(first.value), let scalar = UnicodeScalar(first.value) else { return nil }
    ...
    private static func parseFixedEscape(
        ...
        guard let parsed = parseHexValue(
            in: value,
            afterPrefixAt: index,
    ...
    private static func parseSurrogateTail(in value: String, at index: String.Index) -> HexParse? {
        guard index < value.endIndex,
              value[index] == "\\",
              let parsed = parseHexValue(in: value, afterPrefixAt: index, prefixLength: 2, hexLength: 4),
        ...
        while index < scalars.count {
            ...
            if current.isRegionalIndicator,
               ...
               scalars[index + 1].isRegionalIndicator {
                output.append(contentsOf: "[\(current.regionalIndicatorLetter)\(scalars[index + 1].regionalIndicatorLetter)]".unicodeScalars)
                index += 2
            } else {
